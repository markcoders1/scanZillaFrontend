...
const handleToggleAutoCredits= async()=>{
  try {
    await axiosInstance({ url: appUrl + "/toggleautocredit", method: "GET"}).then(response=>{
     if(response){
  ...
  return (
    <Box>
      <label className={`switch-checkbox ${theme}`}>
        <input value={isChecked} checked={isChecked} onChange={(e)=>{setIsChecked(e.target.checked), handleToggleAutoCredits()}} type="checkbox" />
